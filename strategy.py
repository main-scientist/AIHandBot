...
class Strategy():
    ...
    def strategy(self):
        ...
        if signal == -1:
            ...
            if ((self.enter_price  - price_now) / price_now * 100) >= 0.35 or ((self.enter_price  - price_now) / price_now * 100) <= -2:
                v = (self.bank / (self.enter_price / self.leverage))
                fee_open = (self.enter_price * v) * self.fee / 100
                fee_close = (price_now * v) * self.fee / 100
        ...
        if signal == 1:
            ...
            if ((price_now - self.enter_price) / self.enter_price * 100) >= 0.35 or ((price_now - self.enter_price) / self.enter_price * 100) <= -2:
                v = (self.bank / (self.enter_price / self.leverage))
                fee_open = (self.enter_price * v) * self.fee / 100
                fee_close = (price_now * v) * self.fee / 100
                bank = price_now * (self.bank / (self.enter_price / self.leverage)) - self.enter_price * (self.bank / (self.enter_price / self.leverage)) \
